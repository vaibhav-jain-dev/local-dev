# Health API - Debugger Setup Instructions

This guide explains how to debug the `health-api` service using VSCode or PyCharm.

## Prerequisites

- Docker and Docker Compose running
- `health-api` service started via `make run` in local-dev repo
- VSCode with Python extension OR PyCharm Professional

## Port Configuration

- **Application Port**: 8000
- **Debug Port**: 5678 (mapped to container)
...
The service will start with `debugpy` listening on port 5678.
...
1. Copy it to your health-api workspace:
   ```bash
   mkdir -p .vscode
...
1. Open the `health-api` folder in VSCode
2. Set breakpoints in your code (e.g., `app/views.py`)
3. Press `F5` or go to **Run → Start Debugging**
4. Select **"Python: Remote Attach - health-api"**
...
1. Open the `health-api` project in PyCharm
2. Set breakpoints in your code
3. Go to **Run → Debug → "Debug health-api (Remote)"**
4. Debugger will attach to port 5678
...
Copy the health check code from `repos_docker_files/health-api-health-check.py` to your Django app and add to URLs:
...
2. Check you're debugging the right service (health-api uses port 5678)
...
3. Verify debugpy is installed in container:
   ```bash
   docker exec -it health-api pip list | grep debugpy
   ```
...
```dockerfile
CMD ["python", "-m", "debugpy", "--listen", "0.0.0.0:5678", "--wait-for-client", "manage.py", "runserver", "0.0.0.0:8000", "--noreload"]
```
...
✅ **Port 5678** exposed for debugging
